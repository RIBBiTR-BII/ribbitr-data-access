...
Joins unite rows from two tables along shared key column values, allowing unshared columns between the two tables to be brought into a single table (as we see above in `db_ves_survey`). However, there are different joins to handle key columns vales that are not shared between tables (ie. are unique to one table). The following graphic helps illustrate the different join types:
...
```
**Note:** In this case, the `survey` table is intentionally designed such that all `survey_id`'s in the `ves` table show up in the survey table (definition of a "foreign key" constraint). To confirm this we can try:
...
There are 0 rows in this anti_join. This means that in our Venn diagram, the left lobe is essentially empty or nonexistent. The outcome is that in *this case*, `left_join()` and `inner_join()` are equivalent, as are `full_join()` and `right_join()`. This is true for all foreign key relationships, but is not always true in general.

::: {.callout-tip}
#### Relationship cardinality

Foreign keys which point to primary keys are an example of a many:1 ("many-to-one") relationship This is to say that a foreign key column can hold any number of duplicate values, while a primary key value can show up a maximum of once. Many:1 relationships are most common in our database. Occasionally you may also see a 1:1 relationship or a many:many relationship. This classification of relationships in a relational database is called "cardinality".
...
A final note, the above "lazy table" query construction was created in two steps here, passing the lazy table `db_ves_country` to `data_ves_filter` for subsequent data collection. In general you can break this into as many or as few steps as make sense for the clarity of your code.
