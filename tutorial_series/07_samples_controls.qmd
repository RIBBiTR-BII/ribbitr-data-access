...
ggplot(data_summary_species, aes(x = svl_mm_mean, y = total_peptides_ug_mean, color = taxon_capture)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = total_peptides_ug_min, ymax = total_peptides_ug_max), width = 0) +  # vertical whiskers
  geom_errorbarh(aes(xmin = svl_mm_min, xmax = svl_mm_max), height = 0) +  # horizontal whiskers
  theme_minimal() +
...
Great! But what about our negative controls? We should subtract any peptides found in corresponding negative controls from samples to make this more accurate of the frogss sampled.
...
# we see that a substantial number don't have correspondign negative controls in the database yet (NAs). Let's proceede with those that do
